...
class RetailInvRelHdr extends Model
{
    use HasFactory;
    ...
    public function retailinvreldtls()
    {
        return $this->hasMany(RetailInvRelDtl::class, 'invid', 'invid');
    }
    ...
    public function getMcusmasDoAttribute()
    {
        $tsisnh = \App\Models\Tsisnh::where('trano', $this->donom)
            ->where('formc', $this->dorfc)
            ->where('braco', $this->braco)
            ->first();

        if (!$tsisnh) return null;

        return \App\Models\Mcusmas::where('cusno', $tsisnh->cusno)->first();
    }
