...
<div class="row">
    ...
    <div class="accordion" id="accordionPhase">
        @foreach($mc->mcphase as $i => $phase)
        <div class="accordion-item" id="rowPhase_{{ $i }}">

            <h2 class="accordion-header d-flex align-items-center">
                <button class="accordion-button accordion-title {{ $i > 0 ? 'collapsed' : '' }}" type="button" data-bs-toggle="collapse" data-bs-target="#collapsePhase{{ $i }}">
                    Phase {{ $phase->phase }}
                </button>

                @if($i > 0)
                <button type="button" class="btn btn-danger btn-sm ms-2" onclick="removePhaseRow({{ $i }})">
                    <i class="bi bi-trash"></i>
                </button>
                @endif
            </h2>

            <div id="collapsePhase{{ $i }}"
                ...
                <div class="accordion-body">
                    <div class="row">
                        ...
                        <div class="col-md-6 mt-3">
                            ...
                            <input type="text" name="desc[]" id="desc_{{ $i }}" class="form-control" value="{{ old("desc.$i", $phase->descr) }}">
                        </div>

                        <div class="col-md-6 mt-3">
                            <label class="form-label">Term of Payment (%)</label>
                            <input type="text" name="toppc[]" id="toppc_{{ $i }}" class="form-control" value="{{ old("toppc.$i", $phase->toppc) }}">
                        </div>

                        <div class="col-md-6 mt-3">
                            <label class="form-label">Gross Amount</label>
                            <input type="text" id="gramt_termin_display_{{ $i }}" class="form-control price-display" readonly style="background-color:#e9ecef" value="{{ number_format(old("gramt_termin.$i", $phase->gramt ?? 0), 0, ',', '.') }}">
                            <input type="hidden" name="gramt_termin[]" id="gramt_termin_{{ $i }}" class="price-raw" value="{{ old("gramt_termin.$i", $phase->gramt ?? 0) }}">
                        </div>

                        <div class="col-md-6 mt-3">
                            <label class="form-label">Official Discount</label>
                            <input type="text" id="odisa_termin_display_{{ $i }}" class="form-control price-display" readonly style="background-color:#e9ecef" value="{{ number_format(old("odisa_termin.$i", $phase->odisa ?? 0), 0, ',', '.') }}">
                            <input type="hidden" name="odisa_termin[]" id="odisa_termin_{{ $i }}" class="price-raw" value="{{ old("odisa_termin.$i", $phase->odisa ?? 0) }}">
                        </div>

                        <div class="col-md-6 mt-3">
                            <label class="form-label">Net Amount</label>
                            <input type="text" id="ntamt_termin_display_{{ $i }}" class="form-control price-display" readonly style="background-color:#e9ecef" value="{{ number_format(old("ntamt_termin.$i", $phase->ntamt ?? 0), 0, ',', '.') }}">
                            <input type="hidden" name="ntamt_termin[]" id="ntamt_termin_{{ $i }}" class="price-raw" value="{{ old("ntamt_termin.$i", $phase->ntamt ?? 0) }}">
                        </div>

                        <div class="col-md-6 mt-3">
                            <label class="form-label">VAT ({{ $tax->taxes }}%)</label>
                            <input type="text" id="txamt_termin_display_{{ $i }}" class="form-control price-display" readonly style="background-color:#e9ecef" value="{{ number_format(old("txamt_termin.$i", $phase->txamt ?? 0), 0, ',', '.') }}">
                            <input type="hidden" name="txamt_termin[]" id="txamt_termin_{{ $i }}" class="price-raw" value="{{ old("txamt_termin.$i", $phase->txamt ?? 0) }}">
                        </div>

                        <div class="col-md-6 mt-3">
                            <label class="form-label">Billing Amount</label>
                            <input type="text" id="blamt_termin_display_{{ $i }}" class="form-control price-display" readonly style="background-color:#e9ecef" value="{{ number_format(old("blamt_termin.$i", $phase->blamt ?? 0), 0, ',', '.') }}">
                            <input type="hidden" name="blamt_termin[]" id="blamt_termin_{{ $i }}" class="price-raw" value="{{ old("blamt_termin.$i", $phase->blamt ?? 0) }}">
                        </div>

                        <div class="col-md-6 mt-3">
                            <label class="form-label">Billing Date</label>
                            <span class="text-danger">*</span>
                            <input type="date" name="billd[]" id="billd_{{ $i }}" class="form-control" value="{{ old("billd.$i", $phase->billd) }}" min="{{ $minDate }}" required>
                        </div>
