...
<main id="main" class="main">
  ...
  <section class="section">
    <form id="form-mc" action="{{ route('maintenance_contract.update', $mc->mcid) }}" method="POST">
      @csrf
      @method('PUT')
      ...
      <div class="row">
        ...
        <div class="col-md-6 mt-3">
          ...
          <input type="text" class="form-control" name="refno" id="refno" value="{{ old('refno', $mc->refno) }}" required>
        </div>

        <input type="hidden" name="priod" id="priod" value="{{ old('priod', $mc->priod) }}">

        <div class="col-md-6 mt-3">
          <label for="mcdat" class="form-label">MC Date</label><span class="text-danger"> *</span>
          <input type="date" class="form-control" name="mcdat" id="mcdat" value="{{ old('mcdat', $mc->mcdat) }}" required min="{{ $minDate }}">
        </div>
        
        <div class="col-md-6 mt-3">
          <label for="cuspo" class="form-label">Customer PO</label>
          <input class="form-control" name="cuspo" id="cuspo" value="{{ old('cuspo', $mc->mcnom) }}">
        </div>
        ...
        <div class="col-md-6 mt-3">
          ...
          <select name="depo" id="depo" class="form-control select2" required>
            ...
            @foreach ($depo as $depo)
                <option value="{{ $depo->depo }}" {{ old('depo', $mc->depo) == $depo->depo ? 'selected' : '' }}>
                  {{ $depo->depo }} - {{ $depo->name }}
                </option>
            @endforeach
          </select>
        ...
        <div class="col-md-6 mt-3">
          ...
          <select name="cusno" id="cusno" class="form-control select2" required>
            ...
            @foreach ($customers as $cust)
                <option value="{{ $cust->cusno }}" {{ old('cusno', $mc->cusno) == $cust->cusno ? 'selected' : '' }}>
        ...
        <div class="col-md-6 mt-3">
          ...
          <input class="form-control" name="cusna" id="cusna" value="{{ old('cusna', $mc->cusna) }}" readonly style="background-color:#e9ecef">
        </div>
        
        <div class="col-md-6 mt-3">
          <label for="curco" class="form-label">Currency</label>
          <select class="form-control select2" name="curco" id="curco" value="{{ old('curco', $mc->curco) }}">
            <option value="IDR" {{ old('curco', $mc->curco) == 'IDR' ? 'selected' : '' }}>IDR</option>
            <option value="USD" {{ old('curco', $mc->curco) == 'USD' ? 'selected' : '' }}>USD</option>
          </select>
        </div>
        ...
        <div class="col-md-6 mt-3">
          ...
          <input type="hidden" class="form-control" name="crate" id="crate_raw" value="{{ old('crate', $mc->crate) }}">
        </div>
        ...
        <div class="col-md-6 mt-3">
            ...
            <div class="row g-2">
                <div class="col">
                    <input type="date" class="form-control" name="mcpriods" id="mcpriods" value="{{ old('mcpriods', $mc->gmcfr) }}" required min="{{ $minDate }}">
                </div>

                <div class="col">
                    <input type="date" class="form-control" name="mcpriode" id="mcpriode" value="{{ old('mcpriode', $mc->gmcto) }}" required min="{{ $minDate }}">
                </div>
            </div>
        </div>
        ...
        <div class="col-md-6 mt-3">
            ...
            <input type="text" id="gramt_display" class="form-control total-display" readonly style="background-color:#e9ecef" value="{{ number_format(old('gramt', $mc->gramt ?? 0), 0, ',', '.') }}">
            <input type="hidden" name="gramt" id="gramt" class="price-raw" value="{{ old('gramt', $mc->gramt ?? 0) }}">
        </div>

        <div class="col-md-6 mt-3">
            <label class="form-label">Net Amount</label>
            <input type="text" id="ntamt_display" class="form-control total-display" readonly style="background-color:#e9ecef" value="{{ number_format(old('ntamt', $mc->ntamt ?? 0), 0, ',', '.') }}">
            <input type="hidden" name="ntamt" id="ntamt" class="price-raw" value="{{ old('ntamt', $mc->ntamt ?? 0) }}">
        </div>

        <div class="col-md-6 mt-3">
            <label class="form-label">Down Payment</label>
            <input type="text" id="dpamt_display" class="form-control total-display" readonly style="background-color:#e9ecef" value="{{ number_format(old('dpamt', $mc->dpamt ?? 0), 0, ',', '.') }}">
            <input type="hidden" name="dpamt" id="dpamt" class="price-raw" value="{{ old('dpamt', $mc->dpamt ?? 0) }}">
        </div>

        <div class="col-md-6 mt-3">
            <label class="form-label">VAT ({{$tax->taxes}}%)</label>
            <input type="text" id="txamt_display" class="form-control total-display" readonly style="background-color:#e9ecef" value="{{ number_format(old('txamt', $mc->txamt ?? 0), 0, ',', '.') }}">
            <input type="hidden" name="txamt" id="txamt" class="price-raw" value="{{ old('txamt', $mc->txamt ?? 0) }}">
            <input type="hidden" name="vatax" id="vatax" value="{{$tax->taxes}}">
        </div>

        <div class="col-md-6 mt-3">
            <label class="form-label">MC Amount</label>
            <input type="text" id="blamt_display" class="form-control total-display" readonly style="background-color:#e9ecef" value="{{ number_format(old('blamt', $mc->blamt ?? 0), 0, ',', '.') }}">
            <input type="hidden" name="blamt" id="blamt" class="price-raw" value="{{ old('blamt', $mc->blamt ?? 0) }}">
        </div>

        <div class="col-md-6 mt-3">
          <label for="opten" class="form-label">Option Entry</label>
          <select class="form-control select2" name="opten" id="opten" value="{{ old('opten') }}">
            <option value="1" {{ old('opten', $mc->opten) == '1' ? 'selected' : '' }}>1 - MC Product</option>
            <option value="2" {{ old('opten', $mc->opten) == '2' ? 'selected' : '' }}>2 - MC Term of Payment</option>
          </select>
        </div>
        
        <div class="col-md-6 mt-3">
          <label for="billadr" class="form-label">Billing Address</label>
          <textarea class="form-control" name="billadr" id="billadr" rows="2" readonly style="background-color:#e9ecef">{{ old('billadd', $mc->billadd) }}</textarea>
        </div>

        <div class="col-md-6 mt-3">
            <label class="form-label">NPWP</label>
            <input type="text" id="npwp" name="npwp" class="form-control" value="{{ old('npwp', $mc->npwp) }}" readonly style="background-color:#e9ecef">
        </div>

        <div class="col-md-12 mt-3">
          <label for="intxt" class="form-label">Notes</label>
          <textarea class="form-control" name="intxt" id="intxt" rows="2">{{ old('intxt', $mc->noteh) }}</textarea>
        </div>
      </div>

      <div id="section-detail">
        @include('teknik.maintenance_contract.partial_edit.detail_product')
      </div>

      <div id="section-phase">
        @include('teknik.maintenance_contract.partial_edit.termin_phase')
      </div>
        ...
        <script>
            ...
            $(document).ready(function(){
                ...
                $('.price-raw').each(function(){
                    ...
                    if(value !== ''){
                        raw.closest('.col-md-4, .col-md-6')
                            .find('.price-display, .total-display')
                ...
                let initialCusno = $('#cusno').val();

                if(initialCusno){
                    loadCustomerAddress(initialCusno, true);
                }
            });
            ...
            $(document).ready(function(){
                ...
                if(initialCusno){
                    loadCustomerAddress(initialCusno);
                }
            });
            ...
            $('#cusno').on('change', function(){
                ...
                if(cusno){
                    loadCustomerAddress(cusno);
                }
            });

            function loadCustomerAddress(cusno){
                $.get("{{ route('get-customer-address-mc') }}", { cusno }, function(res){
                    ...
                    $('#cusna').val(res.cusna || res.billcon || '');
            ...
            function fillShiptoOptions(select){
                let selectedValue = select.attr('data-selected') || '';
