...
<div class="row">
  <h4 class="my-2">BBM Detail (OF)</h4>
      ...
      <div class="accordion-item" id="accordion-of-item-{{ $i }}">
        ...
        <div id="details-of-{{ $i }}" class="accordion-collapse collapse {{ $i == 0 ? 'show' : '' }}"
          ...
          <div class="accordion-body">
            <div class="row">
                {{-- INVNO tidak digunakan di OF, tapi backend butuh invno[] -> isi PONO biar aman --}}
                <input type="text" name="invno[]" class="invno-of" id="invno-of-{{ $i }}" value="{{ old('refcno') }}" hidden>
...
<script>
    ...
    $('#warco').on('change', function() {
        ...
        $('select[id^="locco-of-"]').each(function() {
            ...
            $sel.empty().append('<option value="">Pilih Lokasi</option>');
            data.forEach(item => {
            ...
            $sel.html('<option value="">Gagal ambil lokasi</option>').prop('disabled', false);
            });
        });
    });
    ...
    $(document).on('change', 'select[id^="locco-of-"]', function () {
        ...
        .fail(() => {
            ...
            $barangSelect.html('<option value="">Gagal ambil barang</option>');
        })
        .always(() => {
    ...
    $(document).on('change', '.opron-of', function() {
        ...
        $.get(`/get-stobl/${braco}/${warco}/${opron}`, function(data) {
            ...
            $lotSelect.append('<option value="">Tidak ada stok untuk barang ini</option>');
            Swal.fire({
                icon: 'warning',
                title: 'Stok Kosong',
    ...
    window.addOF = function(){
        ...
        <div class="accordion-item" id="accordion-of-item-${i}">
            ...
            <div class="accordion-body">
                ...
                <input type="text" name="invno[]" class="invno-of" id="invno-of-${i}" value="${$('#refcno_of_submit').val()||''}" hidden>
